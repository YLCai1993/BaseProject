...
- (void)getDataFromNetCompleteHandle:(CompletionHandle)completionHandle{
   [XiMaNetManager getRankListWithPageId:_pageId completionHandle:^(RankingListModel* model, NSError *error) {
        if (!error) {
            if (_pageId == 1) {
                [self.dataArr1 removeAllObjects];
            }
            [self.dataArr1 addObjectsFromArray:model.list];
